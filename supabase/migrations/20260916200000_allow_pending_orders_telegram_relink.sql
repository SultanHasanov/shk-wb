...
-- Незавершённая попытка оплаты не делает Telegram-only кабинет ценным.
-- Сохраняем такие заказы за целевым кабинетом, а автоматическую перепривязку
-- блокируем только при успешной оплате или незавершённой/готовой генерации.
...
begin
  ...
  if source_is_technical and (
    exists (
      select 1 from public.payment_orders
       where user_id = source_user_id
         and status in ('waiting_for_capture', 'succeeded')
    )
    ...
    or exists (
      select 1 from public.telegram_generation_requests
       where user_id = source_user_id
