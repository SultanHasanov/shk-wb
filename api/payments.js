...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'download' && req.method === 'GET') {
      ...
      const stat=fs.statSync(installer);res.setHeader('Content-Type','application/vnd.microsoft.portable-executable');res.setHeader('Content-Length',String(stat.size));res.setHeader('Content-Disposition',`attachment; filename="${isCell?'cell-print-1.0.5.exe':'program-windows-1.3.2.exe'}"`);res.setHeader('Cache-Control','private, no-store');return fs.createReadStream(installer).pipe(res);
    }
    if (action === 'download-cell-print' && req.method === 'GET') {
      const installer=path.join(__dirname,'_private','cell-print-installer.exe');
      const stat=fs.statSync(installer);res.setHeader('Content-Type','application/vnd.microsoft.portable-executable');res.setHeader('Content-Length',String(stat.size));res.setHeader('Content-Disposition','attachment; filename="cell-print-1.0.5.exe"');res.setHeader('Cache-Control','no-store');return fs.createReadStream(installer).pipe(res);
    }
